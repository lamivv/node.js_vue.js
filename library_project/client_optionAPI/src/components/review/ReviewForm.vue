      ...
      <div class="card-body container">
          <form>
              ...
              <div class="form-group row">
                  <!-- 별점을 5점 만점으로 각 점수를 Radio 버튼으로 선택하도록 함 -->
...
<path fill="yellow" d="M1728 647q0 22-26 48l-363 354 86 500q1 7 1 20 0 21-10.5 35.5t-30.5 14.5q-19 0-40-12l-449-236-449 236q-22 12-40 12-21 
      ...
      async reviewInsert(){
           let obj = {
            ...
            if(addRes.isSuccessed){
                alert('등록되었습니다.');
                // this.$emit('custmoe-event'[, 전달할 값 ]) : 커스텀 이벤트를 부모에게 전달 
                // 새로운 리뷰가 등록된 걸 'add-review' 라는 이벤트로 부모 컴포넌트에게 전달
        ...
        async reviewUpdate(){
            ...
            if(updateRes.isUpdated){
                alert('수정되었습니다.');
                // this.$emit('custmoe-event'[, 전달할 값 ]) : 커스텀 이벤트를 부모에게 전달 
                // 리뷰가 수정된 걸 'update-review' 라는 이벤트로 부모 컴포넌트에게 전달
        ...
        updateCancel(){
            // this.$emit('custmoe-event'[, 전달할 값 ]) : 커스텀 이벤트를 부모에게 전달 
            // 리뷰가 수정을 취소한 걸 'cancel-edit' 라는 이벤트로 부모 컴포넌트에게 전달
